...
import { analyzeResumeQuality, ResumeData } from "@/lib/ai/index";
...
export async function POST(req: Request) {
    try {
        ...
        const { allowed, isPro } = await checkRateLimit("ai_ats");
        if (!allowed) {
            return new NextResponse(
                JSON.stringify({ 
                    error: "LIMIT_EXCEEDED", 
                    message: isPro 
                        ? "You have reached your daily limit for resume optimization checks." 
                        : "Free users can only run 2 optimization checks per day. Please upgrade to Pro for unlimited access." 
                }), 
        ...
        console.error("[OPTIMIZE_SCORE_ERROR]", error);
