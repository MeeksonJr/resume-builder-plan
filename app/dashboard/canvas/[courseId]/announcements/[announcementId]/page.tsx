...
import { ArrowLeft, Megaphone, User, Calendar, HelpCircle } from "lucide-react";
...
export default async function AnnouncementDetailPage({ params }: AnnouncementDetailPageProps) {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/v1/courses/${courseId}/discussion_topics/${announcementId}`, {
      headers: {
    ...
    if (res.ok) {
      announcementData = await res.json();
    } else {
      fetchError = true;
    }
  } catch (e) {
  ...
  if (fetchError || !announcementData) {
    return (
      <div className="p-8 text-center bg-[#e9eee8] min-h-[50vh] text-[#102b2b]">
        ...
        <p className="text-sm mt-2 text-[#52716a]">Could not retrieve announcement details from the Canvas API.</p>
          ...
          <Card className="rounded-none border-[#102b2b]/15 bg-white shadow-sm overflow-hidden">
            <CardHeader className="border-b border-[#102b2b]/10 bg-[#f7faf5] py-5">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="rounded-none border-[#102b2b]/15 text-[#52716a] text-[9px] uppercase font-bold tracking-wider flex items-center gap-1">
                  <Megaphone className="w-3 h-3 text-[#0d8274]" /> Announcement
                </Badge>
              </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-[#0d8274]">Announcement Message</h3>
                {announcementData.message ? (
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: announcementData.message }}
                  />
                ) : (
                  <p className="text-xs text-[#52716a] italic">No announcement message provided.</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 space-y-4 text-xs">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-[#102b2b]/5 rounded-full flex items-center justify-center border border-[#102b2b]/10">
                  {announcementData.author?.avatar_image_url ? (
                    <img src={announcementData.author.avatar_image_url} alt="author" className="h-10 w-10 rounded-full object-cover" />
                  ) : (
                    <User className="h-5 w-5 text-[#52716a]" />
                  )}
                </div>
                <div>
                  <p className="font-bold text-[#102b2b]">{announcementData.author?.display_name || "Instructor"}</p>
              ...
              <div className="space-y-1 border-t border-[#102b2b]/5 pt-3">
                ...
                <p className="font-extrabold text-[#102b2b]">
                  {new Date(announcementData.posted_at || announcementData.created_at).toLocaleString()}
                </p>
              </div>
            </CardContent>
          </Card>
